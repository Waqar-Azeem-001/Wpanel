...
"Links" column: phases 01–13 predate the crawler; D2 back-fills it for them.

## Design & navigation track

| Phase | Status | Tests | Browser | Links | Commit | Deploy |
|---|---|---|---|---|---|---|
| D0 UI & Navigation Audit | ⬜ | — | — | — | — | — |
...
D0 → D1 → D2 → D3 → 14 Reports → D4 + 15 Admin Operations → 16 Security → 17 Reliability → D5 → 18 → 19
...
| **WHMCS-parity experience, own implementation** (Rule 6) | Familiar workflows without copying licensed code/assets or trademark | Proposed (confirm in D0) |
| **All menus from one registry; all internal links via URL names; link crawler is a CI gate** (Rule 5) | Zero broken links by construction and by test | Proposed (confirm in D0) |
| **Staff area on its own prefix; Django admin reserved for Super Admin emergency use** | Daily work never depends on Django admin | Proposed (confirm in D0) |
| **Design track is presentation-only; never changes URL names, services or API contracts** | Re-skin without regressions | Proposed (confirm in D0) |

------------------------------------------------------------------------

# 28 — CURRENT STARTING TASK

Phases 01–13 are 🟢. The Phase 01 audit is historical and must not be repeated.

## Start: Phase D0 — UI & Navigation Audit

**Do not start coding.** Inspect first (Rule 1).

### Audit Checklist

1. Base templates and layouts that exist; which pages use which.
2. CSS/JS assets and frameworks in use; duplicated styles or components.
3. Complete route inventory: every `urls.py`, URL name, path, view, required permission, area (public/client/staff/API).
4. Every menu, sidebar, dropdown and footer: where it is defined, how links are built.
5. Every template link that is hard-coded, `#`, empty, or `javascript:`.
6. Pages reachable only by typing the URL (not linked from any menu).
7. Menu links that show to users who then get 403.
8. Links in email templates and PDFs: how they are built, which domain they use.
9. Current error pages (403/404/500).
10. Screens that still require Django admin for daily staff work.
11. Mobile overflow and JS errors found on a quick pass of the main screens.
12. Mapping of existing screens to Sections 10 and 11 (exists / partial / missing).

### Required Deliverable — D0 Gap Report

```text
FOUND        What already exists and works.
PARTIAL      Exists but incomplete or inconsistent.
BROKEN       Dead links, 404/500 pages, 403-after-link, JS errors, overflow.
MISSING      Screens/menus from Sections 10–11 that genuinely need building.
REUSE        Existing templates/components to keep and promote to shared components.
ROUTES       Full route inventory table + proposed redirects (no URL name renames).
CHANGES      Exact D1 and D2 changes required.
TEST PLAN    Crawler roles, fixture dataset, lint rules.
EXIT         What must be true before D0 → D1 begins.
```

### After the audit

1. Mark the four "Proposed" decisions in Section 27 as Active (or adjust).
2. D1 → D2 → D3 as in Section 25.
3. Fix any BROKEN link found in D0 immediately, even before D1, with a regression test.
4. Update tracker, deferred items and decisions.
5. Commit only after verification. Do not deploy unless explicitly approved.
